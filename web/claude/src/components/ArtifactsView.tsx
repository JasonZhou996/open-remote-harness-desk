        ...
        {allArtifacts.length === 0 ? (
          ...
          <div className="flex flex-col items-center text-center space-y-6 max-w-md my-auto">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-[#8C8A82] leading-relaxed">{t('If you can dream it, you can build it. Take apps, games, templates, and tools from thought to reality.')}</p>
            </div>
              ...
              {filteredArtifacts.map((art) => (
                ...
                >
                  ...
                  <div className="pt-4 mt-4 border-t border-[#262522] flex items-center justify-between text-xs text-[#7E7C76]">
                    <span>{new Date(art.createdAt).toLocaleDateString(uiLocale())}</span>
                    <div className="flex items-center gap-2">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                      ...
                      <span className="text-[#DA7756] font-medium flex items-center gap-1 group-hover:translate-x-0.5 transition-transform">
                        <Eye className="w-3.5 h-3.5" />{t('Launch')}</span>
                    </div>
                  </div>
                </div>
              ))}
