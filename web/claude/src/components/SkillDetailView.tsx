      ...
      <div className="max-w-5xl mx-auto w-full px-6 sm:px-8 py-6 space-y-5">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6 max-w-3xl pt-2">
            <div>
              <h3 className="text-xs font-medium text-[#706E68] uppercase tracking-wider mb-2">{t('Description')}</h3>
              <p className="text-sm text-[#ECEBE7] leading-relaxed">{skill.description}</p>
            </div>

            <div>
              <h3 className="text-xs font-medium text-[#706E68] uppercase tracking-wider mb-2">{t('Author & Source')}</h3>
              <p className="text-xs text-[#ECEBE7]">{skill.author}</p>
            </div>

            <div>
              <h3 className="text-xs font-medium text-[#706E68] uppercase tracking-wider mb-2">{t('Capabilities Included')}</h3>
              <div className="flex flex-wrap gap-2">
                <span className="text-xs text-[#DA7756] bg-[#22211F] px-2.5 py-1 rounded-lg border border-[#2E2D2A]">{t('Custom Prompts')}</span>
                <span className="text-xs text-[#3B82F6] bg-[#22211F] px-2.5 py-1 rounded-lg border border-[#2E2D2A]">{t('Tool Integration')}</span>
                <span className="text-xs text-emerald-400 bg-[#22211F] px-2.5 py-1 rounded-lg border border-[#2E2D2A]">{t('Memory Sync')}</span>
              </div>
            </div>
          </div>
        )}
                  ...
                  {viewMode === 'raw' ? (
                    <div className="space-y-3 h-full">
                      <div className="text-[11px] text-[#706E68] flex items-center justify-between">
                        <span>{t('Editing mode · changes auto-save')}</span>
                        <button
                          onClick={handleCopy}
