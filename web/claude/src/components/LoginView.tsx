    ...
    <div className="min-h-screen w-full bg-[#141413] text-[#ECEBE7] flex flex-col justify-between select-none">
      ...
      <header className="px-6 py-5 flex items-center justify-between max-w-6xl mx-auto w-full">
        ...
        <div className="flex items-center gap-2 text-xs">
          <span className="px-2.5 py-1 rounded-lg bg-[#201F1D] border border-[#2B2A27] text-[#8C8A82] text-[11px]">{t('Private Workspace')}</span>
        </div>
      </header>
      ...
      <main className="flex-1 flex items-center justify-center p-4 sm:p-6">
        <div className="w-full max-w-md bg-[#1C1B19] border border-[#2E2D2A] rounded-2xl shadow-2xl p-6 sm:p-8 space-y-6">
          ...
          <div className="text-center space-y-2">
            <h1 className="font-serif text-2xl sm:text-3xl font-medium tracking-tight text-[#ECEBE7]">{t('Log in to Claude')}</h1>
            <p className="text-xs sm:text-sm text-[#9C9A92] leading-relaxed">{t('Enter your authorized account email and password to access the workspace.')}</p>
          </div>
          ...
          <div className="flex items-center gap-3 my-4">
            ...
            <span className="text-[11px] font-medium text-[#7E7C76] tracking-wider uppercase">{t('OR')}</span>
            <div className="flex-1 border-t border-[#2B2A27]" />
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-[#B4B3AD]">{t('Email address')}</label>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <label className="text-xs font-medium text-[#B4B3AD]">{t('Password')}</label>
                <button
                  type="button"
      ...
      <footer className="py-4 px-6 text-center text-xs text-[#7E7C76] space-y-1">
        <p>
          ...
          <span className="text-[#A5A39C] hover:underline cursor-pointer">{t('Terms of Service')}</span> and{' '}
          <span className="text-[#A5A39C] hover:underline cursor-pointer">{t('Privacy Policy')}</span>.
        </p>
      </footer>
    </div>
  );
