...
export type ThinkingEffort = 'low' | 'medium' | 'high' | 'extra' | 'xhigh' | 'max' | 'ultracode';
...
export interface Message {
  ...
  isStreaming?: boolean;
  error?: string;
  /** Claude Code transcript row id — anchors edit-send/fork on the backend. */
  transcriptAnchorId?: string;
  /** Present on role:'tool' rows — one tool_use, paired later with its result. */
  toolUse?: {
    toolId: string;
    toolName: string;
    input: unknown;
  };
  /** Attached when the matching tool_result arrives (matched by toolId). */
  toolResult?: {
    content?: string;
    isError?: boolean;
  };
  /** Present while this row is an approval request awaiting a decision. */
  permission?: {
    requestId: string;
    toolName: string;
    input: unknown;
    /** 'pending' until permission_resolved/cancelled lands. */
...
export type ActivePageView = 'routines' | 'chat' | 'projects' | 'artifacts';
