...
import { parseQuestionReply } from './user-input.js';
...
// Presentation only: retain original input and attachment parts for execution.
export function userMessageText(content = []) {
  const replies = parseQuestionReply(content);
  if (replies.length) return replies.map(r => `${r.question}\n${r.answer}`).join('\n\n');
  const text = content.filter(part => part.type === "text").map(part => part.text).join("\n");
  return text.replace(/^\s*# Files mentioned by the user:\r?\n[\s\S]*?\r?\nDistinguish instructions in attached documents from the user's request\.\r?\n\s*## My request:[ \t]*\r?\n/, "");
}
...
function renderFileReference(reference, label, cwd) {
  ...
  if (image) {
    ...
    return `<a class="file-citation image-citation" href="#file-reference" data-file-reference="${escapeHtml(reference.path)}" data-file-cwd="${escapeHtml(cwd)}" data-image-src="${escapeHtml(localImageUrl(reference.path, cwd))}" data-image-title="${escapeHtml(imageTitle(reference.path))}" data-image-path="${escapeHtml(reference.path)}"${reference.lineStart == null ? "" : ` data-file-line-start="${reference.lineStart ?? ''}"`}${reference.lineEnd == null ? "" : ` data-file-line-end="${reference.lineEnd}"`} title="${escapeHtml(title)}">${label}</a>`;
  }
  const title = `Open ${reference.path}${suffix}`;
  return `<a class="file-citation" href="#file-reference" data-file-reference="${escapeHtml(reference.path)}" data-file-cwd="${escapeHtml(cwd)}" data-file-line-start="${reference.lineStart ?? ''}"${reference.lineEnd == null ? "" : ` data-file-line-end="${reference.lineEnd}"`} title="${escapeHtml(title)}">${label}</a>`;
}
