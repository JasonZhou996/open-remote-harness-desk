...
export function reasoningSummaryText(item) {
  if (typeof item.summary === "string") return item.summary;
  return Array.isArray(item.summary) ? item.summary.filter(part => typeof part === "string").join("\n") : "";
}

export function shouldShowActivity(item) {
  return item.type !== "reasoning" || summarizeActivity(item).active;
}
...
export function summarizeActivity(item) {
  ...
  if (item.type === "reasoning") {
    const title = (Array.isArray(item.summary) ? item.summary : [item.summary])
      .filter(part => typeof part === "string")
      .map(part => part.trim().split(/\r?\n/, 1)[0].replace(/^[#*_`]+\s*|\s*[*_`]+$/g, "").trim())
      .findLast(Boolean) || "";
    return { label: active ? title || "Thinking" : "Thought", detail: "", animation: "local-context", active, tone: failed ? "error" : active ? "active" : "success" };
  }
  if (item.type === "mcpToolCall" || item.type === "dynamicToolCall") {
    let args = item.arguments;
    if (typeof args === "string") { try { args = JSON.parse(args); } catch { args = null; } }
    const title = typeof args?.title === "string" ? args.title.trim() : "";
    return { label: active ? "Working" : failed ? "Tool failed" : "Completed", detail: title || item.tool || item.type, animation: "local-context", active, tone: failed ? "error" : active ? "active" : "success" };
  }
