...
Do not publish exploitable details in a public issue. Contact the project maintainer through an existing private channel and include the affected version, reproduction steps, impact, and suggested mitigation if known.

## Security model

三端 Web runs commands and edits files through local Codex, Claude Code, and ZCode runtimes. Treat it as a local developer tool, not as a hardened multi-tenant service.
