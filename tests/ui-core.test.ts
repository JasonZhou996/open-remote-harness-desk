...
import { DEFAULT_TURN_PAGE_SIZE, applyComposerSuggestion, createLatestRequestGate, createTurnWindow, filterAppModels, getComposerAutocomplete, resolveComposerPlaceholder, selectModelState } from "../web/codex/ui-core.js";
...
describe("model selection", () => {
  test("keeps provider models returned by the app server", () => {
    const models = [
      { model: "gpt-5.6-sol", displayName: "GPT-5.6-Sol" },
      { model: "zai/glm-5.3", displayName: "GLM 5.3" },
      { model: "opencode-go/deepseek-v4-flash", displayName: "DeepSeek V4 Flash" },
    ];
    expect(filterAppModels(models).map((model) => model.model)).toEqual([
      "gpt-5.6-sol",
      "zai/glm-5.3",
