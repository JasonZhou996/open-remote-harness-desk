...
} from "../web/codex/codex-surfaces.js";
...
describe("Codex execution state", () => {
  test("shows current thinking and removes finished reasoning", () => {
    expect(shouldShowActivity({type:"reasoning",status:"inProgress",summary:[]})).toBe(true);
    expect(shouldShowActivity({type:"reasoning",status:"completed",summary:[]})).toBe(false);
    expect(shouldShowActivity({type:"reasoning",status:"completed",summary:[" "]})).toBe(false);
    expect(shouldShowActivity({type:"reasoning",status:"completed",summary:["Checking the page"]})).toBe(false);
    expect(reasoningSummaryText({summary:["Checking", "Finished"]})).toBe("Checking\nFinished");
    expect(reasoningSummaryText({content:["private content"]})).toBe("");
    expect(shouldShowActivity({type:"mcpToolCall",status:"completed"})).toBe(true);
  });

  test("keeps descriptive tool titles and actual tool status", () => {
    expect(summarizeActivity({type:"mcpToolCall",tool:"js",status:"inProgress",arguments:{title:"核对手机页面"}})).toMatchObject({label:"Working",detail:"核对手机页面",active:true});
    expect(summarizeActivity({type:"mcpToolCall",tool:"js",status:"completed",arguments:'{"title":"核对手机页面"}'})).toMatchObject({label:"Completed",detail:"核对手机页面",active:false});
    expect(summarizeActivity({type:"dynamicToolCall",tool:"js",status:"failed",arguments:{title:"核对手机页面"}})).toMatchObject({label:"Tool failed",detail:"核对手机页面",tone:"error"});
    expect(summarizeActivity({type:"mcpToolCall",tool:"js",status:"completed",arguments:"{"})).toMatchObject({detail:"js"});
    expect(summarizeActivity({type:"mcpToolCall",tool:"js",arguments:{title:123}})).toMatchObject({detail:"js"});
  });
  ...
  test("uses native exploration verbs for file commands", () => {
    expect(summarizeActivity({ type: "commandExecution", status: "inProgress", command: "cat web/codex/app.js" })).toMatchObject({ label: "Reading", detail: "web/codex/app.js", category: "read", active: true });
    expect(summarizeActivity({ type: "commandExecution", status: "completed", command: "cat web/codex/app.js" })).toMatchObject({ label: "Read", detail: "web/codex/app.js", category: "read", active: false });
  ...
  test("does not disguise compound or mutating commands as exploration", () => {
    expect(summarizeActivity({ type: "commandExecution", status: "inProgress", command: "cat web/codex/app.js && rm web/codex/app.js" })).toMatchObject({ label: "Running command", detail: "cat web/codex/app.js && rm web/codex/app.js", category: "command" });
...
describe("Codex motion constants", () => {
  test("matches animation timing extracted from Codex.app", async () => {
    const { CODEX_MOTION } = await import("../web/codex/codex-surfaces.js");
