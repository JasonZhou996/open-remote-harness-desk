...
await page.evaluate(()=>{
  ...
  api.notify('item/started',{turnId:'native-turn',item:{id:'native-read',type:'commandExecution',status:'inProgress',command:'cat web/codex/app.js'}});
  api.notify('item/completed',{turnId:'native-turn',item:{id:'native-read',type:'commandExecution',status:'completed',command:'cat web/codex/app.js',aggregatedOutput:'source inspected'}});
  ...
  api.notify('turn/completed',{turn:{id:'native-turn',durationMs:null,startedAt:100,completedAt:104,items:[
    ...
    {id:'native-read',type:'commandExecution',status:'completed',command:'cat web/codex/app.js',aggregatedOutput:'source inspected'},
...
if(result.readRow?.text!=='Read web/codex/app.js'||result.readRow.role!==null||result.readRow.ariaExpanded!==null||!result.readRow.labelAndDetailInline||result.readRow.outputText!==''||!result.readRow.outputHidden)throw new Error(`File reads must be a native single-line, non-expandable activity: ${JSON.stringify(result.readRow)}`);
