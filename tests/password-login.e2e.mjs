...
const server=spawn('bun',['run','server/gateway/index.ts'],{
...
const localhost={
  status:await fetch(`http://127.0.0.1:${port}/api/auth/status`).then(response=>response.json()),
  health:await fetch(`http://127.0.0.1:${port}/api/health`).then(response=>response.status),
};
...
// An authenticated HTML cache entry must not be reused for an anonymous visitor.
const localPage=await fetch(`http://127.0.0.1:${port}/`);
const anonymousPage=await fetch(`http://${lanAddress}:${port}/`,{headers:{'if-none-match':localPage.headers.get('etag')}});
if(!(await localPage.text()).includes('data-gateway-authenticated="true"')||anonymousPage.status!==200||(await anonymousPage.text()).includes('data-gateway-authenticated="true"')||anonymousPage.headers.get('cache-control')!=='no-store')throw new Error('HTML authentication state leaked through static cache');
...
console.log(JSON.stringify({localhost,preAuth,initial,wrong,cookie:{httpOnly:cookie?.httpOnly,sameSite:cookie?.sameSite,hasValue:Boolean(cookie?.value)},health,sse,bundle},null,2));
await browser.close();
server.kill('SIGTERM');
