...
import { LiveSessionStore, parseRollout } from "../server/codex/live-session-service.js";
...
describe("Desktop-style live session reader", () => {
  test("reconstructs an active CLI command and its available output", () => {
    ...
    expect(state.turn.items.filter((item: any) => item.type === "userMessage")).toEqual([
      expect.objectContaining({ content: [{ type: "text", text: "Start the task." }] }),
  ...
  test("appends CLI steering and file edits once during incremental reads", () => {
    ...
    try {
      ...
      expect(store.read(path).turn.items).toHaveLength(1);
      expect(store.read(path).turn.items[0].content[0].text).toBe("Initial prompt");
      ...
      expect(store.read(path).turn.items).toHaveLength(3);
    } finally { rmSync(directory, { recursive: true, force: true }); }
  });
});
