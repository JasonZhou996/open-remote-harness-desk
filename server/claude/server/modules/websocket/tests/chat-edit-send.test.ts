...
test('a detached turn reports native failure and cancellation instead of a successful schedule', async () => {
  for (const terminal of [{exitCode:0}, {exitCode:1}, {exitCode:0,aborted:true}]) {
    await withGateway('claude', async () => {
      const result = await runDetachedChatTurn({sessionId:SESSION_ID,userId:1,content:'scheduled turn'}, {runtime:{
        hasRuntime:()=>true,
        run:async (_provider:unknown,_command:unknown,_options:unknown,writer:any)=>{
          writer.send({kind:'complete',provider:'claude',sessionId:SESSION_ID,...terminal});
        },
      } as never});
      assert.equal(result.started,true);
      assert.equal(Boolean(result.error),terminal.exitCode !== 0 || Boolean(terminal.aborted));
    });
  }
});
