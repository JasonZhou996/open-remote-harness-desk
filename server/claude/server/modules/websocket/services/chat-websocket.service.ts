...
async function dispatchRun(
  ...
  try {
    if (provider === 'claude') {
      runtimeOptions.permissionMode = sessionsService.resolvePermissionMode(sessionId, clientOptions.permissionMode);
      sessionsDb.setSessionPermissionMode(sessionId, runtimeOptions.permissionMode);
    }
  ...
  const failed = terminal && (terminal.aborted || terminal.exitCode !== 0);
  return { started: true, error: failure || (failed ? '执行未完成，请查看对话中的错误或取消记录' : null) };
}
