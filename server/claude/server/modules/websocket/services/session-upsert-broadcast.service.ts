...
/** Removes a session from every connected sidebar after persistence succeeds. */
export function broadcastSessionRemoved(sessionId: string, providerSessionId: string | null): void {
  sendToConnectedClients([JSON.stringify({kind: 'session_removed', sessionId, providerSessionId})]);
}
