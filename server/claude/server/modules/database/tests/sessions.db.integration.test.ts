...
async function withIsolatedDatabase(runTest: () => void | Promise<void>): Promise<void> {
  ...
  await writeFile(databasePath, ''); // Prevent legacy DB migration into the test fixture.
...
test('indexing an existing transcript does not restore a removed project', async () => {
  await withIsolatedDatabase(() => {
    sessionsDb.createSession('retained', 'claude', '/workspace/removed', 'Keep history');
    projectsDb.updateProjectIsArchived('/workspace/removed', true);
    sessionsDb.createSession('retained', 'claude', '/workspace/removed', 'Keep history');
    assert.equal(projectsDb.getProjectPath('/workspace/removed')?.isArchived, 1);
    assert.ok(sessionsDb.getSessionById('retained'), 'history is preserved');
    assert.equal(sessionsDb.getRecentSessionsPage(100, 0).total, 0);
    projectsDb.createProjectPath('/workspace/removed');
    assert.equal(projectsDb.getProjectPath('/workspace/removed')?.isArchived, 0, 'explicitly adding the project restores it');
  });
});

test('indexing cannot restore a deleted session even without a file timestamp', async () => {
  await withIsolatedDatabase(() => {
    ...
    assert.equal(sessionsDb.getAllSessions().length, 1, 'explicit restore still works');
  });
});
...
test("createSession leaves an archived row archived when the transcript has not changed", async () => {
  await withIsolatedDatabase(() => {
    ...
    // CLI metadata writes must not undo an explicit deletion either.
...
test("the legacy upsert path also preserves deletion without a timestamp", async () => {
  await withIsolatedDatabase(() => {
    ...
    // Both indexing paths must preserve the user's removal.
