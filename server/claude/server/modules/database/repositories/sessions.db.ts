...
type SessionRow = {
  ...
  effort: string | null;
  /** Last acknowledged session permission choice; legacy sessions remain NULL. */
...
export const sessionsDb = {
   ...
   * Indexing never undoes explicit archival; only the restore action does.
   */
  createSession(
    ...
    // Retracted transcripts remain as backups, but must never replace the
    // canonical branch when a watcher or a full scan discovers them again.
    const superseded = db.prepare(
      'SELECT session_id FROM superseded_provider_sessions WHERE provider_session_id = ? AND provider = ?',
    ...
    // A transcript refresh must not undo an explicit project removal.
  ...
  /** Persists the choice used by live controls and subsequent sends. */
  setSessionPermissionMode(sessionId: string, mode: string): void {
    getConnection().prepare('UPDATE sessions SET permission_mode = ? WHERE session_id = ?').run(mode, sessionId);
  },
