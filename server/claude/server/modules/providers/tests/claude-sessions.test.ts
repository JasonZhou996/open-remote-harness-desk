...
test('SDK streaming envelopes preserve text, thinking and matching completed block identities', () => {
  const provider = new ClaudeSessionsProvider();
  const partial = (index:number, delta:unknown) => provider.normalizeMessage({type:'stream_event',streamMessageId:'api-message',event:{type:'content_block_delta',index,delta}},'session');
  const text = partial(1,{type:'text_delta',text:'hello '});
  assert.equal(text[0]?.kind,'stream_delta');
  assert.equal(text[0]?.content,'hello ');
  assert.equal(text[0]?.streamId,'api-message_1');
  const thought = partial(0,{type:'thinking_delta',thinking:'considering'});
  assert.equal(thought[0]?.kind,'thinking');
  assert.equal(thought[0]?.isDelta,true);
  assert.deepEqual(partial(2,{type:'input_json_delta',partial_json:'{'}),[]);
  const completed = provider.normalizeMessage({type:'assistant',uuid:'cli-uuid',message:{id:'api-message',role:'assistant',content:[{type:'thinking',thinking:'considering'},{type:'text',text:'hello world'}]}},'session');
  assert.equal(completed[1]?.streamId,text[0]?.streamId);
  assert.equal(completed[0]?.streamId,thought[0]?.streamId);
  const singleBlock = provider.normalizeMessage({type:'assistant',streamBlockIndex:1,message:{id:'api-message',role:'assistant',content:[{type:'text',text:'hello world'}]}},'session');
  assert.equal(singleBlock[0]?.streamId,text[0]?.streamId,'CLI one-block snapshots keep their original stream index');
});
...
async function withIsolatedDatabase(runTest: () => void | Promise<void>): Promise<void> {
  ...
  await writeFile(process.env.DATABASE_PATH, ''); // Prevent legacy DB migration into the test fixture.
...
test('named background agents are excluded and old index rows stay archived after rescans', { concurrency: false }, async () => {
  const tmp = await mkdtemp(path.join(os.tmpdir(), 'claude-named-agent-'));
  const projectDir = path.join(tmp, '.claude', 'projects', 'workspace');
  await mkdir(projectDir, { recursive: true });
  const restoreHomeDir = patchHomeDir(tmp);
  try {
    await writeFile(path.join(tmp, '.claude', 'history.jsonl'), '', 'utf8');
    const agentPath = await writeSessionJsonl(projectDir, 'background.jsonl', [
      JSON.stringify({ type: 'agent-name', agentName: 'worker', sessionId: 'test-session-1' }),
    ]);
    const original = await readFile(agentPath, 'utf8');
    await withIsolatedDatabase(async () => {
      const synchronizer = new ClaudeSessionSynchronizer();
      sessionsDb.createSession('test-session-1', 'claude', '/workspace/demo', 'Named worker', undefined, undefined, agentPath);
      assert.equal(await synchronizer.synchronizeFile(agentPath), null);
      assert.equal(sessionsDb.getAllSessions().length, 0);
      assert.equal(sessionsDb.getRecentSessionsPage(100, 0).total, 0);
      assert.equal(await synchronizer.synchronize(), 0);
      assert.equal(sessionsDb.getSessionById('test-session-1')?.isArchived, 1);
      assert.equal(await readFile(agentPath, 'utf8'), original, 'filtering must not modify transcripts');
      // A normal SDK session has the same entrypoint but no native agent marker.
      const mainPath = path.join(projectDir, 'main.jsonl');
      await writeFile(mainPath, JSON.stringify({ type: 'user', sessionId: 'main', cwd: '/workspace/demo', isSidechain: false, entrypoint: 'sdk-ts' })+'\n');
      assert.equal(await synchronizer.synchronizeFile(mainPath), 'main');
      // A sidechain that repeats the parent id must not archive the parent.
      const sidePath = path.join(projectDir, 'sidechain.jsonl');
      await writeFile(sidePath, JSON.stringify({ type: 'user', sessionId: 'main', cwd: '/workspace/demo', isSidechain: true })+'\n');
      assert.equal(await synchronizer.synchronizeFile(sidePath), null);
      assert.equal(sessionsDb.getSessionById('main')?.isArchived, 0);
      assert.equal(sessionsDb.getSessionById('main')?.jsonl_path, mainPath);
      const legacyPath = path.join(projectDir, 'agent-legacy.jsonl');
      await writeFile(legacyPath, await readFile(mainPath));
      assert.equal(await synchronizer.synchronizeFile(legacyPath), null);
    });
  } finally {
    restoreHomeDir();
    await rm(tmp, { recursive: true, force: true });
