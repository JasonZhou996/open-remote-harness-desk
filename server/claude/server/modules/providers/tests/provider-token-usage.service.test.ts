...
test('Claude context uses native model capacity instead of the old fixed 160k limit', async () => {
  const entries = [{type: 'assistant', message: {model: 'native-model', usage: {input_tokens: 100, cache_read_input_tokens: 348500, output_tokens: 200}}}];
  assert.equal(summarizeClaudeTokenUsage(entries, '').total, undefined);
  const service = createProviderTokenUsageService({
    getSessionById: () => createSessionRow({project_path: '/project', jsonl_path: '/transcript'}),
    fileExists: () => true,
    readTextFileTail: async () => ({content: entries.map(e => JSON.stringify(e)).join('\n'), isComplete: true}),
    getClaudeContextWindow: () => '160000',
    resolveClaudeContextWindow: async (cwd, model) => {assert.equal(cwd, '/project'); assert.equal(model, 'native-model'); return 1000000;},
  });
  const usage = await service.getSessionTokenUsage('app-session');
  assert.equal(usage.used, 348800); assert.equal(usage.total, 1000000);
});
