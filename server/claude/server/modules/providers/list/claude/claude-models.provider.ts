...
import { getClaudeCatalog } from '../../services/claude-workspace.service.js';
...
export class ClaudeProviderModels implements IProviderModels {
  async getSupportedModels(): Promise<ProviderModelsDefinition> {
    const { models } = await getClaudeCatalog();
    return {
      DEFAULT: 'default',
      OPTIONS: models.map(model => ({
        value: model.value, label: model.displayName, description: model.description,
        ...(model.supportsEffort && model.supportedEffortLevels?.length ? {
          effort: { default: 'high', values: model.supportedEffortLevels.map(value => ({ value })) },
        } : {}),
      })),
    };
  }
