...
// Manual /usage: boots a dedicated short-lived CLI process — exactly like
// opening the CLI, typing /usage, and quitting — and asks it for plan usage.
const rateLimitStorePath = `${os.homedir()}/.cloudcli/usage-cache.json`;
export async function fetchUsageOnce() {
  let release;
  const held = new Promise((resolve) => { release = resolve; });
  let markBooted;
  const booted = new Promise((resolve) => { markBooted = resolve; });
  const bootBackstop = setTimeout(markBooted, 10000);
  // Async iterable that stays pending until release() — the CLI process boots
  // and waits at an idle prompt, exactly like an open CLI before you type.
  const stream = {
    [Symbol.asyncIterator]: () => ({
      next: () => held.then(() => ({ done: true, value: undefined })),
    }),
  };
  ...
  const drain = (async () => {
    ...
    } catch { /* teardown noise after we release stdin is expected */ }
  })();
  try {
    await booted;
    const answer = await Promise.race([
      instance.usage_EXPERIMENTAL_MAY_CHANGE_DO_NOT_RELY_ON_THIS_API_YET({ skipBehaviors: true }),
      new Promise((_, reject) => setTimeout(() => reject(new Error('usage control timeout')), 30000)),
    ]);
    return answer ?? null;
  } finally {
    release();
...
function mapCliOptionsToSDK(options = {}) {
  ...
  const sdkOptions = { includePartialMessages:true, allowDangerouslySkipPermissions:true };
  ...
  if (settings.skipPermissions && !permissionMode) {
 ...
 */
function addSession(sessionId, queryInstance, writer = null, releaseInput = null, options = null) {
  ...
  activeSessions.set(sessionId, {
    ...
    status: 'active',
    writer,
    options: options || carried?.options,
    changingPermission: carried?.changingPermission || false,
...
async function queryClaudeSDK(command, options = {}, ws, context) {
  ...
  try {
    ...
    if (sessionKey()) {
      addSession(sessionKey(), queryInstance, ws, releasePromptStream, sdkOptions);
    }
    ...
    for await (const message of queryInstance) {
      ...
      if (message.type === 'system' && message.permissionMode && !parent) {
        sdkOptions.permissionMode = message.permissionMode;
        ws.send(createNormalizedMessage({kind:'status',provider:'claude',sessionId:capturedSessionId || sessionId,
          text:'permission_mode',permissionMode:message.permissionMode}));
      }
      if (message.type === 'stream_event') {
        if (message.event?.type === 'message_start') streamBlocks.set(parent, {id:message.event.message.id,index:0});
        const block = streamBlocks.get(parent);
        if (block && message.event?.type === 'content_block_start') block.index = message.event.index;
        transformedMessage.streamMessageId = block?.id;
      } else if (message.type === 'assistant' && message.message?.content?.length === 1) {
        // The CLI emits a one-block assistant snapshot before content_block_stop,
        // so its array index is always zero even for the second API block.
        const block = streamBlocks.get(parent);
        if (block?.id === message.message.id) transformedMessage.streamBlockIndex = block.index;
      }
      ...
      if (message.type === 'result') {
        ...
        if (!turnCompleteSent && !abortPending) {
          ...
          ws.send(createCompleteMessage({ provider: 'claude', sessionId: capturedSessionId || sessionId || null, exitCode: message.is_error ? 1 : 0 }));
          notifyRunStopped({
            userId: ws?.userId || null,
            provider: 'claude',
            sessionId: sessionId || capturedSessionId || null,
...
export const claudeRuntime = {
  ...
  permissions: {
    ...
    async setMode(sessionId, mode) {
      ...
      if (session.changingPermission) throw new Error('正在切换权限，请稍后重试。');
      session.changingPermission = true;
      try {
        await session.instance.setPermissionMode(mode);
        if (getSession(sessionId)?.instance !== session.instance) throw new Error('本轮已结束，请重新选择权限。');
        // canUseTool reads this same object; do not leave its bypass check stale.
        session.options.permissionMode = mode;
        return true;
      } finally {
        session.changingPermission = false;
        const current = getSession(sessionId);
        if (current?.instance === session.instance) current.changingPermission = false;
      }
    },
  },
};
