...
export class ClaudeSessionsProvider implements IProviderSessions {
  ...
  private normalizeMessageRows(rawMessage: unknown, sessionId: string | null): NormalizedMessage[] {
    ...
    if (raw.type === 'stream_event') {
      ...
      const streamId = raw.streamMessageId && event && Number.isInteger(event.index) ? `${raw.streamMessageId}_${event.index}` : undefined;
      if (event?.type === 'content_block_delta') {
        if (event.delta?.type === 'text_delta') return [createNormalizedMessage({kind:'stream_delta',content:event.delta.text,streamId,sessionId,provider:PROVIDER})];
        if (event.delta?.type === 'thinking_delta') return [createNormalizedMessage({kind:'thinking',content:event.delta.thinking,isDelta:true,streamId,sessionId,provider:PROVIDER})];
      }
      if (event?.type === 'content_block_stop') return [createNormalizedMessage({kind:'stream_end',streamId,sessionId,provider:PROVIDER})];
      return [];
    }
        ...
        for (const part of raw.message.content) {
          if (part.type === 'text' && part.text) {
            messages.push(createNormalizedMessage({
              ...
              streamId: raw.message.id ? `${raw.message.id}_${raw.streamBlockIndex ?? partIndex}` : undefined,
              sessionId,
              timestamp: ts,
              provider: PROVIDER,
              kind: 'text',
