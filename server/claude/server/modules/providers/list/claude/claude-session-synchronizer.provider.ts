...
export class ClaudeSessionSynchronizer implements IProviderSessionSynchronizer {
  ...
  private isSubagentTranscript(filePath: string): boolean {
    ...
    return pathParts.includes('subagents') || pathParts.includes('tool-results') || /^agent-.*\.jsonl$/.test(path.basename(filePath));
  }
  ...
  private async processSessionFile(
    ...
    const metadata = await this.readSessionMetadata(filePath, parsed.sessionId);
    if (metadata?.isSubagent) {
      // Named background agents also live beside ordinary transcripts. Hide
      // old index entries without deleting files or archiving a parent row.
      if (existingSession?.jsonl_path === filePath) {
        sessionsDb.updateSessionIsArchived(existingSession.session_id, true);
      }
      return null;
    }
    ...
    let sessionName = metadata?.title;
    if (!sessionName) {
  ...
  /**
   * Reads the title and native agent markers, including named background agents
   * stored at the project root with isSidechain=false.
   *
  ...
  private async readSessionMetadata(
    filePath: string,
    sessionId: string
  ): Promise<{ title?: string; isSubagent: boolean } | undefined> {
    try {
      ...
      for (let index = 0; index < lines.length; index += 1) {
        ...
        if (data.isSidechain === true || (eventType === 'agent-name' && typeof data.agentName === 'string' && data.agentName.trim())) {
      ...
      return { title: foundCustomTitle || foundAiTitle || foundLastPrompt, isSubagent: false };
    } catch {
