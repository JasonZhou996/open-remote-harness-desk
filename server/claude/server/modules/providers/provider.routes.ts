...
import { fetchUsageOnce } from '@/modules/providers/list/claude/claude-runtime.provider.js';

import { sessionImportService } from './services/session-import.service.js';
import {claudeStatistics} from './services/claude-statistics.service.js';
import { claudeWorkspacePath, getClaudeCatalog, readClaudeInstructions, writeClaudeInstructions, readClaudeMemory } from './services/claude-workspace.service.js';
...
/**
 * Real subscription plan + rate-limit windows for the logged-in Claude
 * account: forwards the CLI's stored OAuth token to Anthropic's usage
 * endpoint and adds the plan tier from the credentials file.
 */
// CLI-style usage source: each conversation run fires one get_usage control
// request (the CLI's /usage) while its CLI process is live and persists the
// answer to usage-cache.json (see claude-runtime.provider.js). This route only
// serves that snapshot — zero polling, zero extra upstream calls.
router.get(
  '/claude/usage',
  asyncHandler(async (_req: Request, res: Response) => {
    ...
    try {
      ...
      plan = raw?.claudeAiOauth?.subscriptionType ?? plan;
    } catch { /* credentials unreadable — plan stays unknown */ }
    const storePath = `${process.env.HOME ?? ''}/.cloudcli/usage-cache.json`;
    const serveCache = async (): Promise<boolean> => {
      try {
        const { readFile } = await import('node:fs/promises');
        const stored = JSON.parse(await readFile(storePath, 'utf8'));
        if (stored?.usage) {
          res.json(createApiSuccessResponse({ plan: typeof stored.plan === 'string' ? stored.plan : plan, usage: stored.usage }));
          return true;
        }
      } catch { /* no captured snapshot yet */ }
      return false;
    };
    if (await serveCache()) return;
    res.json(createApiSuccessResponse({ plan, error: 'usage awaits the first conversation turn' }));
  }),
);

// Manual /usage: boots a dedicated short-lived CLI process (like typing /usage
// in the CLI and quitting), persists the answer, and returns it.
router.post(
  '/claude/usage/refresh',
  asyncHandler(async (_req: Request, res: Response) => {
    const answer = await fetchUsageOnce();
    const limits = answer?.rate_limits;
    if (!limits || (!limits.five_hour && !limits.seven_day)) {
      throw new AppError('The usage control request returned no rate limits.', {
    ...
    const snapshot = {
      plan: typeof answer.subscription_type === 'string' ? answer.subscription_type : plan,
      usage: { five_hour: limits.five_hour || null, seven_day: limits.seven_day || null },
      captured_at: new Date().toISOString(),
    };
    try {
      const { writeFile } = await import('node:fs/promises');
      await writeFile(`${process.env.HOME ?? ''}/.cloudcli/usage-cache.json`, JSON.stringify(snapshot, null, 2));
    } catch { /* best-effort persistence */ }
    res.json(createApiSuccessResponse(snapshot));
  }),
);

router.get('/claude/statistics', asyncHandler(async (req,res)=>{ const days=Number(req.query.days || 0); if(![0,7,30].includes(days))throw new AppError('时间范围无效',{statusCode:400,code:'INVALID_RANGE'}); res.json(createApiSuccessResponse(await claudeStatistics(days))); }));

router.get('/claude/workspace', asyncHandler(async (req, res) => {
  const cwd = claudeWorkspacePath(readOptionalQueryString(req.query.projectId));
  res.json(createApiSuccessResponse({ cwd, ...await getClaudeCatalog(cwd, req.query.refresh === '1') }));
}));
router.get('/claude/instructions', asyncHandler(async (req, res) => {
  res.json(createApiSuccessResponse(await readClaudeInstructions(String(req.query.scope || 'user'), readOptionalQueryString(req.query.projectId))));
}));
router.put('/claude/instructions', asyncHandler(async (req, res) => {
  res.json(createApiSuccessResponse(await writeClaudeInstructions(String(req.body.scope || 'user'), readOptionalQueryString(req.body.projectId), req.body.content, req.body.revision)));
}));
router.get('/claude/memory', asyncHandler(async (req, res) => {
  res.json(createApiSuccessResponse(await readClaudeMemory(readOptionalQueryString(req.query.projectId))));
}));
...
// ----------------- Session routes -----------------
router.get('/sessions/import-candidates', asyncHandler(async (req, res) => {
  const projectPath = readPathParam(req.query.projectPath, 'projectPath');
  res.json(createApiSuccessResponse(await sessionImportService.candidates(projectPath)));
}));
router.post('/sessions/import', asyncHandler(async (req, res) => {
  const sessionId = readPathParam(req.body?.sessionId, 'sessionId');
  const projectPath = readPathParam(req.body?.projectPath, 'projectPath');
  res.json(createApiSuccessResponse(await sessionImportService.register(sessionId, projectPath)));
}));
/**
...
router.post(
  '/sessions/:sessionId/retract',
  asyncHandler(async (req: Request, res: Response) => {
    const sessionId = parseSessionId(req.params.sessionId);
    const anchorId = typeof req.body?.anchorId === 'string' ? req.body.anchorId.trim() : '';
    if (!SESSION_ID_PATTERN.test(anchorId)) throw new AppError('有效的消息 anchorId 是必填项。', {code:'INVALID_ANCHOR_ID', statusCode:400});
    res.json(createApiSuccessResponse(await sessionsService.retractMessage(sessionId, anchorId)));
  }),
);

router.get('/sessions/:sessionId/permission-mode', asyncHandler(async (req:Request,res:Response) => {
  res.json(createApiSuccessResponse({mode:sessionsService.resolvePermissionMode(parseSessionId(req.params.sessionId))}));
}));
router.post('/sessions/:sessionId/permission-mode', asyncHandler(async (req:Request,res:Response) => {
  if (typeof req.body?.mode !== 'string') throw new AppError('权限模式是必填项。', {code:'INVALID_PERMISSION_MODE',statusCode:400});
  res.json(createApiSuccessResponse(await sessionsService.setPermissionMode(parseSessionId(req.params.sessionId),req.body.mode)));
}));
