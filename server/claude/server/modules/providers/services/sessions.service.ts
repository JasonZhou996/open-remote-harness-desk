...
import { broadcastSessionUpserted, broadcastSessionRemoved, chatRunRegistry, connectedClients } from '@/modules/websocket/index.js';
...
export const sessionsService = {
  /** Shared validation and saved default for REST controls and websocket sends. */
  resolvePermissionMode(sessionId: string, requested?: unknown): string {
    const session = sessionsDb.getSessionById(sessionId);
    if (!session) throw new AppError('会话不存在。', {code:'SESSION_NOT_FOUND',statusCode:404});
    if (session.provider !== 'claude') throw new AppError('当前提供方不支持权限切换。', {code:'UNSUPPORTED_PROVIDER',statusCode:400});
    const mode = requested === undefined ? session.permission_mode ?? 'acceptEdits' : requested;
    if (typeof mode !== 'string' || !['default','acceptEdits','plan','bypassPermissions','auto','dontAsk'].includes(mode)) {
      throw new AppError('无效的权限模式。', {code:'INVALID_PERMISSION_MODE',statusCode:400});
    }
    return mode;
  },

  /** Wait for the real SDK control acknowledgement before saving the new mode. */
  async setPermissionMode(sessionId: string, requested: unknown) {
    const mode = this.resolvePermissionMode(sessionId, requested);
    const run = chatRunRegistry.getRun(sessionId);
    const applied = await providerRegistry.resolveProvider('claude').runtime.permissions?.setMode?.(sessionId, mode) ?? false;
    if (!applied && run?.status === 'running') throw new AppError('Claude 进程正在启动，请稍后再切换权限。', {code:'RUNTIME_NOT_READY',statusCode:409});
    sessionsDb.setSessionPermissionMode(sessionId, mode);
    if (applied) run?.writer.send({kind:'status',provider:'claude',text:'permission_mode',permissionMode:mode,sessionId});
    return {mode,appliedToActiveRun:applied};
  },
  /**
  ...
  /** Used by the retract route: rewind context without submitting another prompt. */
  async retractMessage(sessionId: string, anchorId: string): Promise<FetchHistoryResult> {
    const source = sessionsDb.getSessionById(sessionId);
    if (!source) throw new AppError('会话不存在。', {code:'SESSION_NOT_FOUND', statusCode:404});
    if (source.provider !== 'claude') throw new AppError('当前提供方不支持撤回。', {code:'RETRACT_NOT_SUPPORTED', statusCode:409});
    const provider = providerRegistry.resolveProvider('claude');
    // Reserve the same slot as chat.send before any asynchronous lookup/fork.
    const run = chatRunRegistry.startRun({appSessionId:sessionId, provider:'claude', providerSessionId:source.provider_session_id, connection:null, userId:null});
    if (!run) throw new AppError('请先停止当前回复，再撤回消息。', {code:'SESSION_BUSY', statusCode:409});
    let succeeded = false;
    try {
      const history = await this.fetchHistory(sessionId);
      const target = history.messages.find(m => m.role === 'user' && (m.transcriptAnchorId ?? m.id) === anchorId);
      const anchor = target && await provider.sessions.resolveEditAnchor?.(sessionId, anchorId);
      if (!anchor?.found) throw new AppError('消息尚未保存或已经撤回，请刷新后重试。', {code:'ANCHOR_NOT_FOUND', statusCode:409});
      if (!source.provider_session_id || !source.jsonl_path || !provider.fork) throw new AppError('会话记录尚未就绪。', {code:'RETRACT_NOT_READY', statusCode:409});
      const forked = anchor.resumeThroughId === null ? null : await provider.fork.forkSession({
        providerSessionId:source.provider_session_id, jsonlPath:source.jsonl_path,
        projectPath:source.project_path ?? '', upToAnchorId:anchor.resumeThroughId, title:source.custom_name ?? undefined,
      });
      // An abort while the SDK was reading must not overwrite a newer run.
      if (chatRunRegistry.getRun(sessionId) !== run || run.status !== 'running') throw new AppError('撤回已取消，请重试。', {code:'RETRACT_CANCELLED', statusCode:409});
      getConnection().transaction(() => {
        sessionsDb.markProviderSessionSuperseded({providerSessionId:source.provider_session_id!, provider:'claude', sessionId, jsonlPath:source.jsonl_path});
        if (forked) sessionsDb.repointSessionToProviderSession(sessionId, forked);
        else sessionsDb.detachProviderSession(sessionId);
      })();
      const result = await this.fetchHistory(sessionId);
      connectedClients.forEach(connection => run.writer.updateWebSocket(connection));
      run.writer.send({kind:'history_truncated', provider:'claude', sessionId, anchorId, retracted:true});
      succeeded = true;
      return result;
    } finally {
      chatRunRegistry.completeRunIfCurrent(run, {exitCode:succeeded ? 0 : 1});
    }
  },
  ...
  async deleteOrArchiveSessionById(
    ...
    if (!options.force) {
      ...
      broadcastSessionRemoved(sessionId, session.provider_session_id);
      return {
        sessionId,
        action: 'archived',
  ...
  async renameSessionById(sessionId: string, summary: string): Promise<{ sessionId: string; summary: string }> {
    const session = this.getSessionDetailsById(sessionId);
    sessionsDb.updateSessionCustomName(session.sessionId, summary);
    await broadcastSessionUpserted(session.sessionId);
    return { sessionId: session.sessionId, summary };
  },
};
