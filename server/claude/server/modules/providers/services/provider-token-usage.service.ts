...
import { getClaudeContextWindow } from './claude-workspace.service.js';
...
type ProviderTokenUsageServiceDependencies = {
  ...
  resolveClaudeContextWindow: (cwd: string, model: string) => Promise<number>;
...
const defaultDependencies: ProviderTokenUsageServiceDependencies = {
  ...
  resolveClaudeContextWindow: getClaudeContextWindow,
...
export function createProviderTokenUsageService(
  ...
  return {
    ...
    async getSessionTokenUsage(sessionId: string): Promise<TokenUsageResult> {
      ...
      const usage = summarizeClaudeTokenUsage(entries, dependencies.getClaudeContextWindow());
      if (usage.model) {
        // The bridge's legacy CONTEXT_WINDOW is a fixed estimate, not this model's capacity.
        usage.total = undefined;
        try {
          usage.total = await dependencies.resolveClaudeContextWindow(session.project_path || dependencies.getHomeDirectory(), usage.model);
        } catch {
          // Keep the measured count when native capacity is unavailable; never invent a percentage.
          usage.message = '上下文窗口上限暂不可用';
        }
      }
      return usage;
    },
