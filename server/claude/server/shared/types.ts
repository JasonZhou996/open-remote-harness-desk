...
export type NormalizedMessage = {
  id: string;
  /** API message id plus content-block index, shared by partial and full output. */
  streamId?: string;
  /** True for incremental thinking; a full thinking block replaces its partials. */
  isDelta?: boolean;
  /** Acknowledged live permission mode, carried by a permission_mode status. */
...
export type ProviderRuntimePermissionGateway = {
  ...
  /** Change an active SDK query and await its acknowledgement; false if idle. */
  setMode?(sessionId: string, mode: string): Promise<boolean>;
};
