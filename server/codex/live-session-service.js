...
function messageItem(payload, fallbackId) {
  if (payload?.type !== "message" || payload.role !== "assistant") return null;
  const text = contentText(payload.content);
  if (!text) return null;
  return { id: payload.id || `host-message-${fallbackId}`, type: "agentMessage", text, phase: payload.phase || "final_answer" };
}

function reasoningItem(payload, fallbackId) {
  if (payload?.type !== "reasoning") return null;
  const summary = contentText(payload.summary);
  if (!summary) return null;
  return { id: payload.id || `host-reasoning-${fallbackId}`, type: "reasoning", summary: [summary], status: "completed" };
}
...
export function applyRolloutEvent(state, event) {
  ...
  const fallbackId = `${state.turn.id}-${++state.responseSequence}`;
  const message = messageItem(payload, fallbackId);
  if (message) { upsertItem(state, message); return state; }
  const reasoning = reasoningItem(payload, fallbackId);
...
export function rolloutActivity(path) {
  const fd = openSync(path, "r");
  try {
    const {size, mtimeMs} = fstatSync(fd), stamp = `${size}:${mtimeMs}`;
    if (activityCache.get(path)?.stamp === stamp) return activityCache.get(path).active;
    let end = size, suffix = "", active = null;
    // ponytail: bound sidebar IO to 8 MiB per changed rollout; unknown stays unknown.
    while (end > 0 && size - end < 8 * 1024 * 1024) {
      const start = Math.max(0, end - 65536), bytes = Buffer.alloc(end - start);
      readSync(fd, bytes, 0, bytes.length, start);
      const lines = (bytes.toString("utf8") + suffix).split("\n");
      suffix = start ? lines.shift() : "";
      for (let i = lines.length - 1; i >= 0; i--) {
        if (!/"type"\s*:\s*"(?:task_started|task_complete|turn_aborted)"/.test(lines[i])) continue;
        try {
          const event = JSON.parse(lines[i]);
          if (event.type !== "event_msg") continue;
          const type = event.payload?.type;
          if (!["task_started", "task_complete", "turn_aborted"].includes(type)) continue;
          active = type === "task_started";
          break;
        } catch {}
      }
    ...
    activityCache.set(path, {stamp, active});
    return active;
  } finally {closeSync(fd);}
}
